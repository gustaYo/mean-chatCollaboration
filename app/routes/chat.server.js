...
module.exports = function(app, express, controllers, io) {
    ...
    io.sockets.on('connection', function(socket) {
        ...
        socket.on('nickname', function(user, fn) {
            ...
            nicknames[user.username] = _user;
            socket.join('all');
            fn(_user);
            controllers.chat.getRoomsUser(socket.nickname, function(salas) {
        ...
        socket.on('getlog conver', function(userTab, fn) {
            controllers.chat.getLogConver(socket.nickname, userTab, fn)
        ...
        socket.on('addGrupo', function(data, fn) {
            controllers.chat.addGroupUser(data, fn)
        });
        socket.on('ordenaGrupo', function(data, fn) {
            controllers.chat.ordenaGrupo(data, fn)
        });
        socket.on('addUserGroup', function(data, fn) {
            controllers.chat.addUserGroup(data, function(err, p) {
                // notificar a usuarios conectados de peticion de amistad
                if (data.peti) {
                    controllers.user.getUser(nicknames[socket.nickname]._id, function(user) {
                        for (var i in data.usersadd) {
                            var post = getUserSocket(data.usersadd[i]);
                            if (post != -1)
                                io.sockets.socket(nicknames[post].socket).emit("peticionAmistad", user);
                        }
                    })
                }
                fn(null, nicknames)
        ...
        socket.on('cancelPeti', function(data, fn) {
            controllers.chat.cancelPeti(data, fn)
        });
        ...
        socket.on('scannerFtp', function(ftp, fn) {
            controllers.ftp.ScannerFTP(ftp, socket, fn)
        });
...
//        }
    ...
    function getUserSocket(idUser) {
        for (var i in nicknames) {
            if (nicknames[i]._id == idUser)
                return i;
        }
        return -1;
    }
